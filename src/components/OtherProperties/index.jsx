	...
	render() {
		...
		if (nearby && nearby.length > 0) {
			nearByBuildings = nearby.map((building) => {
				...
				return (
					<Carrousel key={building.name} name={building.name} areaName={areaName} coverImage={coverImage} />
				);
			});
		}

		const preLoder = LoadersArr.map((n) => <Carrousel coverImage={n.src} key={n.name} />);

		return (
			<div className="slider-container" onMouseEnter={this.showArrow} onMouseLeave={this.hideArrow}>
				<Slider {...settings}>{nearby && nearby.length > 0 ? nearByBuildings : preLoder}</Slider>
			</div>
		);
	}
}
